...
export class BusinessAnalyzer {
  ...
  constructor() {
    ...
    if (!this.isEnabled) {
      logger.warn(
        "OpenAI API key not found - business analysis will use mock data",
      );
    } else {
      logger.info(
        "Hunter Brody Business Analyzer initialized with real AI analysis",
      );
    }
  }
  ...
  async analyzeBusinessIntelligence(
    ...
    try {
      ...
      const models = ["gpt-4o", "gpt-4-turbo", "gpt-3.5-turbo"]; // Use proven stable models
      ...
      for (const model of models) {
        try {
          ...
          usedModel = model;
          logger.info("✅ OpenAI model succeeded", {
            model,
            businessName: input.businessName,
          });
          break;
      ...
      const result: BusinessAnalysisResult = {
        ...
        optimizationOpportunities: this.validateArray(
          analysisData.optimizationOpportunities,
        ),
        riskFactors: this.validateArray(analysisData.riskFactors),
        recommendedTools: this.validateArray(analysisData.recommendedTools),
        marketPosition:
          analysisData.marketPosition || "Position analysis pending",
        competitiveAdvantage: this.validateArray(
          analysisData.competitiveAdvantage,
        ),
        growthPotential:
          analysisData.growthPotential || "Growth potential assessment pending",
      ...
      throw new Error(
        `AI analysis failed: ${isTimeout ? "Request timeout" : errorMessage}`,
      );
    }
  }
...
Focus on practical, implementable solutions that can drive immediate ROI for this business.
  ...
  private generateMockAnalysis(
    ...
    logger.info("🎭 Generating mock business analysis", {
      businessName: input.businessName,
    });
    ...
    return {
      ...
      marketPosition:
        "Established player with digital transformation opportunities",
      competitiveAdvantage: [
      ...
      growthPotential:
        "Moderate to high growth potential through strategic digital investments",
      nextSteps: [
